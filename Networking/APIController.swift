...
class APIController {
    ...
    public class func makeRequestReturnJSON(_ request: API, completion:@escaping ((JSONDictionary?,Int, APIError?,Data) -> Void)) {
        APIController.callService(request: request) { (result) in
            DispatchQueue.main.async {
                ...
                case .success(let response):
                    guard let json = try? response.mapJSON() as? JSONDictionary else {
                        ...
                        completion(nil,response.statusCode ,APIError.unknownError(),Data())
                        return
                    }
...
//                        }
//                    }
                    completion(json,response.statusCode,nil,response.data)
                case .failure(let error):
                    completion(nil,0,error,Data())
                }
            }
